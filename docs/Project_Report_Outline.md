...
The proposed project, **Admission Counsellor Follow-up Dashboard**, is a full-stack web application designed to manage the complete admission funnel from enquiry to conversion. The system allows parents to login with email, submit enquiries, send parent portal updates, assigns leads to counsellors, tracks each status change, logs follow-up calls, schedules demo tours, manages referrals, tracks seat availability, and provides analytics for centre heads. It also includes a rule-based AI layer that generates counsellor call scripts, follow-up messages, priority flags, and operational recommendations without using any paid external AI API.

The main objective of the project is to improve admission conversion by reducing missed follow-ups and increasing accountability. Counsellors can quickly view their leads, update statuses, and log call outcomes. Admin users can monitor all leads, counsellor-wise performance, overdue follow-ups, demo bookings, funnel movement, teacher/daycare/classroom updates, referrals, and seat availability. The project is built using React, Vite, Tailwind CSS, Node.js, Express, MySQL, and JWT authentication, making it practical for deployment through Vercel, Render, and Railway.
...
- Functional requirements: staff login, parent email login, lead CRUD, enquiry submission, parent portal updates, school operations records, follow-ups, tours, referrals, seat availability, analytics, notifications.
- Non-functional requirements: responsive UI, secure JWT auth, role-based admin access, database integrity.
- Use cases: parent enquiry submission, parent portal update, teacher/daycare/classroom record entry, counsellor follow-up, admin monitoring, demo booking, referral tracking, seat availability update.

## Chapter 4: System Design

- Architecture: React frontend, Express API, MySQL database, rule-based AI service, deployment layer.
- ER diagram: counsellors, parents, leads, follow_ups, tour_slots, tour_bookings, admissions, notifications, workflow_events, seat_availability, referrals.
- API design: REST endpoints with standard success/error responses.

## Chapter 5: Implementation

- Frontend modules: login, parent login, parent portal, dashboard, leads, lead detail, enquiry, tours, operations, analytics.
- Backend modules: auth routes, lead routes, follow-up routes, operations routes, analytics routes, AI engine.
- Key code snippets: JWT middleware, priority calculation, tour booking transaction, operational recommendation logic.
...
The project also demonstrates how rule-based AI can be useful even without paid external APIs. The call script generator, follow-up message generator, priority calculation engine, and operational recommendation logic support counsellors and centre heads with practical suggestions while keeping the system low-cost and easy to explain during review. The use of JWT authentication, role-based access, MySQL relational schema, and deployment-ready configuration makes the solution suitable as a realistic internship prototype.

Future improvements can include WhatsApp Business API integration for automated reminders, email notifications, fee payment tracking, advanced admission forecasting, downloadable PDF reports for centre heads, and parent-facing admission status tracking. The system can also be extended to support multiple branches, lead source ROI tracking, teacher mobile apps, and OpenAI-powered conversation summaries if API access is approved later.
